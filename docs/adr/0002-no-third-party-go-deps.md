...
| `golang.org/x/crypto/x509roots/fallback` | Separate module (same maintainers) that embeds Mozilla NSS roots and calls `x509.SetFallbackRoots` at init. Linked via [`internal/tlsroots`](../../internal/tlsroots/) so cgo Windows release binaries can verify `chat.signal.org` when the OS pool is empty. |
