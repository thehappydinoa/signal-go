...
sequenceDiagram
    autonumber
    actor User
    participant Phone as Signal app (primary)
    participant Bot as signal-go (secondary)
    ...
    Bot-->>Server: 200 OK
    Note over Bot: render<br/>sgnl://linkdevice?uuid=…&pub_key=…
    ...
    Bot->>Server: PUT /v1/devices/link<br/>(AccountAttributes,<br/>signed + Kyber last-resort prekeys)
...
- **Steps 1–6** are the unauthenticated provisioning websocket. Signal
  ...
  exists yet.
- **Step 9 (ECDH + HKDF + AES-CBC + HMAC)** is the
...
- **Step 11 (prekey generation)** mints two namespaces' worth of keys:
  ACI and PNI. PQXDH is mandatory upstream, so the Kyber/ML-KEM last-
  resort prekey ships at link time.
- **Steps 13–16** are two separate REST calls. The link succeeds at the
  first; the second populates the one-time prekey batches recipients
