...
// libsignal exposed this as the `SignalPROFILE_KEY_LEN` macro through
// v0.97.2; v0.101.2 dropped it in favor of an anonymous FixedArray32
// typedef on the FFI signatures, so this is now a literal (confirmed
// against signal_ffi.h).
...
func ProfileKeyVersion(profileKey []byte, aci string) (string, error) {
	...
	if err := checkError(C.signal_service_id_parse_from_service_id_string(&sid, cInt8(cstr))); err != nil {
		return "", err
	}
	var pk [ProfileKeyLen]C.uchar
	copyProfileKey(&pk, profileKey)
