...
func (k *MessageBackupKey) AesKey() ([32]byte, error) {
	...
	if err := checkError(C.signal_message_backup_key_get_aes_key(
		(*C.SignalType_FixedArray32_uint8_t)(unsafe.Pointer(&out)),
		C.SignalConstPointerMessageBackupKey{raw: k.raw.raw},
	)); err != nil {
		return [32]byte{}, err
	}
	var key [32]byte
...
func (k *MessageBackupKey) HmacKey() ([32]byte, error) {
	...
	var out [32]C.uint8_t
	if err := checkError(C.signal_message_backup_key_get_hmac_key(
		(*C.SignalType_FixedArray32_uint8_t)(unsafe.Pointer(&out)),
		C.SignalConstPointerMessageBackupKey{raw: k.raw.raw},
	)); err != nil {
		return [32]byte{}, err
