...
func TestLinkDeviceSendsExpectedRequest(t *testing.T) {
	...
	resp, err := c.LinkDevice(context.Background(), "+15551234567", "passw0rd", req)
	if err != nil {
	...
	// Auth username is the e164 number; the provisioning code is in the body.
	wantAuth := "Basic " + base64.StdEncoding.EncodeToString([]byte("+15551234567:passw0rd"))
	if gotAuth != wantAuth {
...
func TestLinkDeviceSurfacesServerErrors(t *testing.T) {
	...
	defer srv.Close()
	c := New(srv.URL, "test")
	_, err := c.LinkDevice(context.Background(), "+15551234567", "pwd", LinkDeviceRequest{VerificationCode: "code"})
...
func TestLinkDeviceValidatesInputs(t *testing.T) {
	...
	if _, err := c.LinkDevice(context.Background(), "", "pw", LinkDeviceRequest{VerificationCode: "code"}); err == nil {
		t.Error("expected error empty number")
	}
	if _, err := c.LinkDevice(context.Background(), "+15551234567", "", LinkDeviceRequest{VerificationCode: "code"}); err == nil {
		t.Error("expected error empty password")
	}
	if _, err := c.LinkDevice(context.Background(), "+15551234567", "pw", LinkDeviceRequest{}); err == nil {
		t.Error("expected error empty verification code")
	}
}
